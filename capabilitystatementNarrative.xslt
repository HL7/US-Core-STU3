...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://hl7.org/fhir" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://www.w3.org/1999/xhtml" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:fn="http://hl7.org/fhir/xslt-functions" exclude-result-prefixes="xs xhtml fn">
				...
				<div xmlns="http://www.w3.org/1999/xhtml">
					...
					<xsl:if test="fhirVersion/@value|acceptUnknown/@value|format/@value|profile/@value">
						...
						<table>
							<tbody>
								...
								<xsl:if test="profile/reference/@value">
									<tr>
										<th>Supported profiles:</th>
										<td>
											<xsl:for-each select="profile/reference/@value">
												<xsl:variable name="var" select="replace(upper-case(substring-before(substring-after(., 'definition-'),'.html')),'-' ,' ')"/>
												<p>
													<a href="{.}">
					...
					<xsl:for-each select="rest">
						...
						<table class="grid">
							...
							<tbody>
								<xsl:for-each select="resource">
									<tr>
										<th>
											<xsl:variable name="var1" select="type/@value"/>
											<xsl:for-each select="profile/reference/@value">
												<xsl:variable name="var2" select="replace(upper-case(substring-before(substring-after(., 'definition-'),'.html')),'-' ,' ')"/>
												<a href="{.}">
						...
						<xsl:if test="operation">
							<p>
								...
								<xsl:for-each select="operation">
									...
									<xsl:for-each select="extension[@url='http://hl7.org/fhir/StructureDefinition/capabilitystatement-expectation']/valueCode/@value">
						...
						<xsl:if test="interaction">
							<xsl:variable name="doCapabilityStatement" as="xs:boolean" select="exists(interaction/extension[@url='http://hl7.org/fhir/StructureDefinition/capabilitystatement-expectation']/valueCode/@value)"/>
	...
	<xsl:template name="doParams" as="element(xhtml:table)">
		<xsl:variable name="doCapabilityStatement" as="xs:boolean" select="exists(*[self::searchParam or self::parameter]/extension[@url='http://hl7.org/fhir/StructureDefinition/capabilitystatement-expectation']/valueCode/@value)"/>
		<table class="list">
			<thead>
				<tr>
					<th>Parameter</th>
			...
			<tbody>
				<xsl:for-each select="searchParam|parameter">
                 ...
                 <a href="{definition/@value}">-->
						...
						<xsl:if test="$doCapabilityStatement">
							<td>
								<xsl:value-of select="extension[@url='http://hl7.org/fhir/StructureDefinition/capabilitystatement-expectation']/valueCode/@value"/>
							</td>
						</xsl:if>
						<td>
	...
	<xsl:template name="doCapabilityStatement" as="node()">
		...
		<xsl:variable name="value" as="xs:string">
			<xsl:choose>
				<xsl:when test="extension[@url='http://hl7.org/fhir/StructureDefinition/capabilitystatement-expectation']">
					<xsl:value-of select="extension[@url='http://hl7.org/fhir/StructureDefinition/capabilitystatement-expectation']/valueCode/@value"/>
				</xsl:when>
