...
1.  One status in `Observation.status`which has an [extensible](http://hl7.org/fhir/STU3/terminologies.html#extensible) binding to:
...
1.  Either one `Observation.valueQuantity` or, if there is no value, one code in `Observation.DataAbsentReason`
    -   Each Observation.valueQuantity must have:
        ...
        -   a UCUM unit code in `Observation.valueQuantity.code` which has an [extensible](http://hl7.org/fhir/STU3/terminologies.html#extensible) binding to:
...
1.  When using component observations (Note: See
    ...
    1.  Either one `Observation.component.valueQuantity` or, if there is
          ...
          -   Each Observation.component.valueQuantity must have:
              ...
              -   a UCUM unit code in
                  `Observation.component.valueQuantity.code` which has an [extensible](http://hl7.org/fhir/STU3/terminologies.html#extensible) binding to:
                   -   [Vital Signs Units] value set.
...
1.  When using related observations, one or more reference
 ...
 [Vital Signs]: ValueSet-us-core-observation-ccdavitalsignresult.html
  [Vital Signs Units]: http://hl7.org/fhir/STU3/valueset-ucum-vitals-common.html
